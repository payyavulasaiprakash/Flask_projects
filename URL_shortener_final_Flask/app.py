...
def shorten_url():
    ...
    return r''.join(random.choice(letters) for i in range(6)) #can changed as required
...
def home():
    if request.method == "POST":
        ...
        if url_received.endswith("/"):
            url_received=url_received[:-1]
        hashid=shorten_url()
        ...
        db.session.commit()
        print(shortened_url)
        return render_template('homepage.html',url_to_be_added=shortened_url,url_received=url_received)
    else:
        url_to_be_added=""
        return render_template('homepage.html',url_to_be_added=url_to_be_added,url_received=url_to_be_added)
...
def copy_url():
    if request.method=="POST":
        ...
        if original_url:
            return f"Your input URL <h3>{original_url.input_url}</h3> got copied."
        else:
